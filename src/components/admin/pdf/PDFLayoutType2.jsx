...
export default function PDFLayoutType2({ applicant }) {
    ...
    const CheckBox = ({ label, checked, textSize="text-[11px]" }) => (
        <div className="flex items-center gap-1.5">
            <div className={`w-3 h-3 border border-slate-600 flex items-center justify-center shrink-0 rounded-[1px] ${checked ? 'bg-slate-200' : 'bg-white'} mt-[1px]`}>
                {checked && <div className="w-1.5 h-1.5 bg-slate-800 rounded-[0.5px]" />}
            </div>
            <span className={`${textSize} text-slate-900 leading-4`}>{label}</span>
        </div>
    );
            ...
            <div className="border border-slate-800 p-3 mb-2 rounded-sm relative">
                ...
                <div className="grid grid-cols-3 gap-x-1 gap-y-1 text-[11px]">
                    ...
                    <div className="flex items-center gap-1.5 col-span-1">
                         ...
                         <span className="text-[11px] text-slate-900 leading-4">หนังสือยินยอมเปิดเผยข้อมูลฯ (PDPA)</span>
                    </div>
